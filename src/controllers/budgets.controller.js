...
export const countBudget = async (req, res) => {
    const today = new Date();
    const year = today.getFullYear();
    const month = String(today.getMonth() + 1).padStart(2, '0');
    const day = String(today.getDate()).padStart(2, '0');
    
    const basePath = path.join(__dirname, '..', '..', 'presupuestos');
    const dayFile = path.join(basePath, `presupuestos.json`);
    ...
    try {
        ...
        res.status(200).send({totalBudgets: fileData[`${day}-${month}-${year}`].count.toString()});
    } catch (error) {
